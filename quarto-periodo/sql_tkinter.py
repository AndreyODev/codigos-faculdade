...
def insert_aluno():
    ...
    if nome and idade.isdigit() and curso:
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO alunos (nome, idade, curso) VALUES (%s, %s, %s)", (nome, int(idade), curso))
            conn.commit()
            messagebox.showinfo(
                "Sucesso", "Dados do aluno inseridos com sucesso!")
        ...
        messagebox.showwarning(
            "Erro de entrada", "Preencha todos os campos corretamente.")
...
def insert_professor():
    ...
    if nome and disciplina:
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO professores (nome, disciplina) VALUES (%s, %s)", (nome, disciplina))
            conn.commit()
            messagebox.showinfo(
                "Sucesso", "Dados do professor inseridos com sucesso!")
        except psycopg2.Error as e:
            messagebox.showerror("Erro no banco", f"{e.pgcode} - {e.pgerror}")
        finally:
            if conn:
...
tk.Button(root, text="Cadastrar Aluno",
          command=insert_aluno).grid(row=3, columnspan=2)
...
tk.Button(root, text="Cadastrar Professor",
          command=insert_professor).grid(row=6, columnspan=2)
...
tk.Button(root, text="Mostrar Dados", command=fetch_data).grid(
    row=11, columnspan=2)
